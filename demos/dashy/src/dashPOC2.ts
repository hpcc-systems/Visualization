// tslint:disable
import { Dashboard } from "@hpcc-js/marshaller";
...
export const dashboard = Dashboard.create("placeholder", ddl as any);

/*
dashboard
    .renderPromise().then(() => {
        dashboard.restore(ddl as any);
        dashboard.renderPromise().then(() => {
            return ec.refresh();
        }).then(() => {
            for (const error of ec.validate()) {
                console.log(error.elementID + " (" + error.source + "):  " + error.msg);
            }
        });
    })
    ;
