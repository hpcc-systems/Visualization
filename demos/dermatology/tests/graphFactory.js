...
(function (root, factory) {
    ...
    var DataGraphVerticesColumns = ["id", "label", "group", "centroid", "ann1", "ann2", "ann3"];
    var DataGraphVertices = DataGraphVerticesRaw.map(v => ([...v, v[1] === "Valjean", Math.random() > .8, Math.random() > .8, Math.random() > .8]));
    var DataGraphEdges = [
        ["1->0", 1, 0, 1],
        ["2->0", 2, 0, 8],
        ["3->0", 3, 0, 10],
    ...
    return {
        Graph2: {
            ...
            LesMis: function (callback) {
                legacyRequire(["src/graph/DataGraph", "src/graph/AnnotationColumn"], function (DataGraph, AnnotationColumn) {
                    const graph = new DataGraph()
                        .categories(Array(11).fill().map((row, idx) => ({ id: "" + idx, faChar: "fa-user" })))
                        .annotations(Array(5).fill().map((row, idx) => ({ id: "" + idx, faChar: "fa-plus" })))
            ...
            LesMis_1: function (callback) {
                legacyRequire(["src/graph/DataGraph"], function (DataGraph) {
                    const graph = new DataGraph()
                        .categories([{ id: 1, faChar: "A" }, { id: 2, faChar: "B" }])
