...
export class Radar extends SVGWidget {
    protected polar2cartesian = (r, theta) => [r * Math.cos(theta), r * Math.sin(theta)];
    protected cartesian2polar = (x, y) => [Math.atan2(y, x), Math.sqrt(x * x + y * y)];
    protected degrees2radians = d => d / 57.295779513;
    protected radians2degrees = r => r * 57.295779513;
    protected point_in_polygon = (point, vs) => {
        const x = point[0];
        const y = point[1];
        let inside = false;
    ...
    protected measureText = (font_style, text) => {
    ...
    update(domNode, element) {
        ...
        const data = this.flattenData(this.columns(), this.data()).map(function (d: any) {
            d.shape = context.mapShape(context.pointShape());
            d.column = context.columns()[d.colIdx];
            return d;
        });
        ...
        let max_label_w = 0;
        const font_style = `${this.fontSize()}px '${this.fontFamily()}'`;
        data.filter(n => n.colIdx === 1).forEach(n => {
            const len = context.measureText(font_style, n.label);
            if (max_label_w < len)max_label_w = len;
        });
        ...
        labels.enter()
            ...
            .each(function (this: SVGElement, n, i) {
                ...
                text_anchor = Math.round(xy[0]) > 0 ? "start" : text_anchor;
                text_anchor = Math.round(xy[0]) < 0 ? "end" : text_anchor;
                d3Select(this)
