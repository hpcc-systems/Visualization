...
export class RadialBar extends SVGWidget {
    ...
    enter(domNode, element) {
        ...
        this.axialAxis = this.svg.append('g')
            .attr('class', 'a axis')
            ;
        this.radialAxis = this.svg.append('g')
            .attr('class', 'r axis')
            ;
    ...
    update(domNode, element) {
        ...
        let scale = d3scaleLinear()
            .domain([0, Math.max.apply(undefined, this.data().map(d => d[1])) * 1.1])
            .range([0, this.maxRange()]);

        let ticks = scale.ticks(this.tickCount());
        ...
        this.radialAxis
            ...
            .enter().append('g')
            ;
        this.radialAxis.append('circle').classed('circle1', true);

        this.svg.selectAll('.circle1')
        ...
        this.svg.selectAll('.text1')
            .attr('x', -5)
            .attr('y', (d, i) => -this.getOuterRadius(i) + this.arcPadding())
            .text(d => d.name)
            ;

        let gTicks = this.axialAxis
            .selectAll('g')
            .data(ticks)
            .enter().append('g')
            .classed("g-tick", true)
        ...
        this.svg.selectAll('.arc')
            ...
            .attrTween('d', (d, i) => {
                let interpolate = d3interpolate(0, d[1]);
...
RadialBar.prototype.publish("radiusPadding", 40, "number", "Padding between chart edge and container edge (px)", RadialBar.prototype._palette.switch());
