...
export class SummaryC extends CanvasWidget {
    ...
    update(domNode, element) {
        ...
        const _main_font_size = _size.height * Math.pow(_mult, 1);
        const _sub_font_size = _size.height * Math.pow(_mult, 3);
        const _icon_size = _size.height - (_size.height * Math.pow(_mult, 4));
        const _px = _size.width * Math.pow(_mult, 6);
        const _py = _size.height * Math.pow(_mult, 6);
