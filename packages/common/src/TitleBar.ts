...
export class TitleBar extends IconBar {
    ...
    update(domNode, element) {
        ...
        const innerHeight = this.fixedHeight() - (this.buttonMargin() * 2);
        const titleSize = this.description_exists() ? innerHeight * (1 - this.descriptionSizeRatio()) : innerHeight;
        const descriptionSize = innerHeight * this.descriptionSizeRatio();
        this._divTitleIcon
            .text(this.titleIcon())
            .style("font-family", this.titleIconFont())
            .style("font-size", `${innerHeight}px`)
...
export interface TitleBar {
    ...
    descriptionSizeRatio(_: number): this;
}
TitleBar.prototype.publish("descriptionSizeRatio", 0.32, "number");
