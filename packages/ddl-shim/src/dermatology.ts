...
export function dermatologyUpgrade(ddl2: any, dermObj: any) {
    apply_to_dataviews(ddl2, dermObj);
    apply_to_properties_layout(ddl2, dermObj);
}

function apply_to_dataviews(ddl2: any, dermObj: any) {
    ...
    function apply_to_dataview(dv: any) {
        ...
        function apply_class_mapping(dermWidgetObj: any) {
            ...
            function swap_with_supported_class(_class: string): string {
                ...
                return classMap[_class] ? classMap[_class] : _class;
            }
        }
        function apply_panel_property_mapping(dermPanelProps: any, dermWidgetProps: any) {
            const excludeList: any = [];
            dv.visualization.title = dermPanelProps.title ? dermPanelProps.title : "";
            ...
            for (const propName in dermPanelProps) {
                ...
                } else if (excludeList.indexOf(propName) === -1) {
                    dv.visualization.properties[propName] = dermPanelProps[propName];
                }
            }
        ...
        function apply_widget_property_mapping(dermPanelProps: any, dermWidgetProps: any) {
            const excludeList: any = [];
            dv.visualization.title = dermWidgetProps.title ? dermWidgetProps.title : "";
            ...
            for (const propName in dermWidgetProps) {
                if (typeof propertyMappings[propName] !== "undefined") {
                    ...
                    // TODO - should non-mapped properties be added to dv.visualization.properties by default?
                    dv.visualization.properties[propName] = dermWidgetProps[propName];
                }
            }
        }
    }
...
function apply_to_properties_layout(ddl2: any, dermObj: any) {
    ...
    dermObj.__properties.content.forEach((cell: any) => {
        ...
        ddl2.properties.layout.push(cellPosition);
    });
}
