...
function getConfig(env) {
    // env is set by the 'buildEnvironment' and/or 'environment' plugin options (see vite.config.ts),
    // or by the code at the end of this file if used standalone
    const dojoConfig = {
        baseUrl: ".",
        packages: [
            {
...
// Export the config.  This is needed both at build time (bundlers) and on the client at runtime
// for the packed application.
if (typeof module !== "undefined" && module) {
    module.exports = getConfig;
} else {
    // No bundler.  This script was loaded by page via script tag, so load Dojo from CDN
