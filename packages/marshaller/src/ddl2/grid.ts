...
export class DashboardGrid extends Grid implements IClosable, IVizPopupPanelOwner, IDockPanel {
    ...
    syncWidgets() {
        ...
        no_layout_added.forEach(cp => {
            const hSpan = Math.ceil(maxY / 3);
            this.setContent(maxY, 0, cp, "", hSpan, maxX);
            maxY += hSpan;
        });
    ...
    surfacePadding_default: { (): string; (_: string): DashboardGrid; };
    surfaceBorderWidth_default: { (): number; (_: number): DashboardGrid; };
