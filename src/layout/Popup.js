...
(function (root, factory) {
    ...
    Popup.prototype.publish("enableDragging", false, "boolean", "Enable/Disable dragging",null,{});
    ...
    Popup.prototype.publish("position", "fixed", "set", "Value of the 'position' property",["absolute", "relative", "fixed", "static", "initial", "inherit" ],{tags:['Private']});
    ...
    Popup.prototype.update = function (domNode, element) {
        ...
        if(this.enableDragging()){
            this._element.call(d3.behavior.drag()
                .on("dragstart", function () {
                    d3.event.sourceEvent.stopPropagation();
                    context._startLoc = [d3.event.sourceEvent.clientX,d3.event.sourceEvent.clientY];
                })
                .on("drag", function () {
                    context._deltaX = d3.event.sourceEvent.clientX - context._startLoc[0];
                    context._deltaY = d3.event.sourceEvent.clientY - context._startLoc[1];
                    context._element
                        .style("left",(context.left() + context._deltaX) + "px")
                        .style("top",(context.top() + context._deltaY) + "px")
                    ;
                })
                .on("dragend", function () {
                    d3.event.sourceEvent.stopPropagation();
                    if(typeof (context._deltaX) !== 'undefined' && typeof (context._deltaY) !== 'undefined'){
                        context
                            .left(context.left() + context._deltaX)
                            .top(context.top() + context._deltaY)
                        ;
                        context._deltaX = undefined;
                        context._deltaY = undefined;
                        context.render();
                    }
                }));
        } else {
            this._element.on(".drag", null);
        }
    };
