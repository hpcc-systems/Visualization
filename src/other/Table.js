...
(function (root, factory) {
    ...
    Table.prototype.enter = function (domNode, element) {
        ...
        this.tableDiv = element.append("div").classed("tableDiv", true);
                ...
                fixedRows
                    ...
                    .each(function(d, i) {
                        if (rows[0][i - 1] && (d3.select(rows[0][i - 1]).classed("selected")) === true) {
                            if (rows[0][i - 1].parentElement.querySelector(":hover") === rows[0][i - 1]) {
                                d3.select(this).classed("selected hover", true);
                            } else {
                                d3.select(this).classed("selected", true);
