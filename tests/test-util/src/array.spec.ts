...
describe.only("Array", function () {
    ...
    it("compare", function () {
        ...
        expect(diff.unchanged.length).to.equal(0);
        expect(diff.removed.length).to.equal(baseArr.length);

        newArr = [3, 777];
        diff = compare(baseArr, newArr);
        expect(diff.added.length).to.equal(0);
        expect(diff.unchanged.length).to.equal(2);
        expect(diff.unchanged).to.deep.equal([3, 777]);
        expect(diff.removed.length).to.equal(baseArr.length - 2);
        expect(diff.removed).to.deep.equal([1, 2, 4, 5, 66, 8888, 9, 0]);

        newArr = [3, 666, 8888];
        diff = compare(baseArr, newArr);
        expect(diff.added.length).to.equal(1);
        expect(diff.added).to.deep.equal([666]);
        expect(diff.unchanged.length).to.equal(2);
        expect(diff.unchanged).to.deep.equal([3, 8888]);
        expect(diff.removed.length).to.equal(baseArr.length - 2);
        expect(diff.removed).to.deep.equal([1, 2, 4, 5, 66, 777, 9, 0]);
    });
    it("compare2", function () {
        ...
        diff = compare(baseArr, newArr);
        expect(diff.added.length).to.equal(0);
        expect(diff.unchanged.length).to.equal(1);
        expect(diff.removed.length).to.equal(baseArr.length - 1);
        ...
        diff = compare2(baseArr, newArr, row => row.id);
        expect(diff.added.length).to.equal(0);
        expect(diff.unchanged.length).to.equal(3);
        expect(diff.removed.length).to.equal(baseArr.length - 3);
