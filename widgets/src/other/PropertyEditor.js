...
(function(root, factory) {
            ...
            table.select("tbody").each(function(widget, widgetIdx) {
                ...
                if (context._param_grouping === "By Param") {
                    ...
                    rows.enter().append("tr").each(function(d) {
                        ...
                        tr.append("td").attr("class", "label").html(function(sProp) {
                            var text = '';
                            switch (sProp.rowType) {
                                case 'shared':
                                    text = sProp.description;
                                    break;
                                case 'individual':
                    ...
                    //Setting the td.label html content
                    rows.selectAll('td.label').each(function(sProp) {
                        var text = '';
                        switch (sProp.rowType) {
                            case 'shared':
                                text = sProp.description;
                ...
                //Updating TR 'By Widget'
                    ...
                    rows.enter().append("tr").each(function(d) {
                                ...
                                tr.append("td").attr("class", "label").text(function(d) {
    ...
    function addClass(element, klass) {
        if (typeof (element.className) === 'string') {
            var class_arr = element.className.split(' ');
            if (class_arr.indexOf(klass) === -1) {
                class_arr.push(klass);
    ...
    function removeClass(element, klass) {
        if (typeof (element.className) === 'string') {
            ...
            var revised_arr = [];
            if (class_arr.indexOf(klass) !== -1) {
                class_arr.forEach(function(n, i) {
                    if (n !== klass) {
                        revised_arr.push(n);
                    }
                });
                element.className = revised_arr.join(' ');
            }
        } else {
            element.className = '';
        }
